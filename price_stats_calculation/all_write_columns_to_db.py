...
for file in file_list:
    ...
    df_filtered_1 = df_filter_na.filter(df_filter_na['zipcode'].isNotNull())
    df_filtered_2 = df_filtered_1.filter(df_filtered_1['zipcode'].isNotNull())
    df_zipcode_formatted = df_filtered_2.withColumn('zipcode', df_filtered_2['zipcode'].cast(IntegerType()))
    ...
    df_tsp = df_tsp_renamed.withColumn('month', month(df_tsp_renamed['timestamp']))
    df_final = df_tsp.withColumn('date', df_tsp['timestamp'].cast('date'))
    df_final.show()
